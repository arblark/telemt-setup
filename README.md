...
- **Готовые профили** — выбор конфигурации одной цифрой:
  - **Рекомендуемый** — Secure (dd) + Middle Proxy + TLS-маскировка
  - **Максимальная маскировка** — TLS (ee) + полная TLS-эмуляция
  - **Простой** — Classic + прямое подключение + mask_host (проверенная рабочая конфигурация)
...
| # | Профиль | Режим | Middle Proxy | Маскировка | Для кого |
|---|---|---|---|---|---|
| 1 | **Рекомендуемый** | Secure (dd) | Да | TLS-эмуляция | Большинство случаев |
| 2 | **Максимальная маскировка** | TLS (ee) | Да | Полная TLS-эмуляция | Сильная цензура (Иран, Китай) |
| 3 | **Простой** | Classic | Нет | mask_host (cloudflare.com) | Проверенная рабочая конфигурация |
...
| Secure | true | `TM_MODE_SECURE` | С `dd`-префиксом (по умолчанию) |
| TLS | false | `TM_MODE_TLS` | С `ee`-префиксом + SNI |
...
──────────────────────────────────────────────────────────
  Ссылки для подключения в Telegram (Secure/dd):
